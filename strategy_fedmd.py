...
class FEDMD_Strategy(FedAvg):
    def __init__(self,
        ...
        self.num_samples = len(self.serverloader.dataset) if server_loader else 0
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]):
        ...
        np_param_results = np.array(param_results).squeeze(1)
        
        # Make sure this function is defined somewhere
        aggregated_ndarrays = average_client_logits(np_param_results, num_clients=self.num_clients, num_samples=5000)
...
#             print(f'Length of results: {len(param_results)}')

#             np_param_results = np.array(param_results)

            

#             aggregated_ndarrays = average_client_logits(np_param_results, num_clients = self.num_clients, num_samples=self.num_samples)
